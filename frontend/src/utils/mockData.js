...
export const demoFullAnalysis = {
  ...
  talentDNA: '你的五大才干构成了一个非常罕见的组合——成就与责任共同筑起了你强大的执行底座：你不仅有内驱力驱动自己不断完成，还有使命感确保你对承诺负责到底。分析才干为这一切注入了理性之光，让你在行动前能看清问题的本质。而体谅与沟通则赋予了你温度，使你的执行力不是冷冰冰的机器，而是能让周围人感到被理解、被看见的力量。这种"有头脑、有行动力、有温度"的组合，在职场中极为稀缺。',
  crossDomainInsight: '你的执行域（成就+责任）与战略域（分析）的交叉是你最大的职场优势。大多数人要么擅长想、要么擅长做，而你两者兼备：分析才干让你在行动前透彻理解问题，成就与责任才干确保你把洞察真正变成结果。建议在团队中主动承担那些需要"既要分析清楚、又要落地执行"的复杂任务，这正是你才干组合最能发光的舞台。',
  // backward-compat
